...
def get_parser():
    ...
    parser.add_argument(
        "--dataset",
        default="VSL",
        choices=["WLASL100", "WLASL300", "WLASL1000",
                 "WLASL2000", "AUTSL", "SLR500", "VSL"],
        help="dataset name",
    )
    parser.add_argument(
        "--save-score",
...
class Processor:
    ...
    def load_optimizer(self):
        if self.arg.optimizer == "SGD":
            ...
            self.optimizer = optim.SGD(
                params, momentum=0.9, nesterov=self.arg.nesterov)
    ...
    def train(self, epoch, save_model=False):
        ...
        if save_model:
            ...
            torch.save(
                save_dict, self.arg.model_saved_name +
                "epoch-" + str(epoch) + ".pt"
            )
        ...
        with torch.no_grad():
            ...
            for ln in loader_name:
                ...
                if accuracy > self.best_acc:
                    ...
                    torch.save(
                        save_dict, self.arg.model_saved_name + "best_model.pt")
                ...
                self.print_log(
                    f"recall of every test dataset class:\n{total_recall}")
                self.print_log(
                    f"precision of every test dataset class:\n{total_precision}")
                self.print_log(
                ...
                for k in self.arg.show_topk:
                    self.print_log(
                        "\tTop{}: {:.2f}%".format(
                            k, 100 *
                            self.data_loader[ln].dataset.top_k(score, k)
                        )
                    )
                    self.print_log(
    ...
    def start(self):
        if self.arg.phase == "train":
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if save_model:
                    val_loss = self.eval(
                        epoch, save_score=self.arg.save_score, loader_name=[
                            "test"]
                    )
